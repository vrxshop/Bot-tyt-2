...
def get_payment_request(request_id: int):
    ...
    try:
        ...
        if response.data:
            ...
            logging.info(f"✅ Заявка #{request_id} найдена: {data}")
            # Возвращаем кортеж
            return (
                data['id'],          # 0
                data['user_id'],     # 1
...
⚠️ <b>Переводить на вайлдберриз (ВБ) банк!</b>
...
📌 Вы получаете 60% от каждой покупки вашего реферала. НАВСЕГДА!
Тоесть вы пожизненно получаете 60% от ВСЕХ покупок вашего реферала в боте
💸 Минимальная сумма для вывода: 300 ₽ 
"""
    },
    "en": {
...
TARIFFS = {
    ...
    "11": {
        ...
        "duration_days": None,
        "category": "main",
        "desc_ru": "✅Этот тариф включает в себя все тарифы бота, вы получите весь контент которого вам хватит на ОЧЕНЬ ДОЛГО🔮\n✅Также этот тариф выдается навсегда, один раз купили и пользуетесь всегда"
     },
     "12": {
...
class AdminStates(StatesGroup):
    ...
    waiting_for_withdraw_user = State()      # <-- ДОЛЖНО БЫТЬ ТАК!
    waiting_for_withdraw_amount = State()    # <-- ДОЛЖНО БЫТЬ ТАК!
...
def get_admin_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💸 Списать баланс", callback_data="admin_withdraw_balance")],
...
async def reject_payment(callback: CallbackQuery):
    ...
    # Извлекаем ID заявки
    request_id_str = callback.data.replace("reject_payment_", "")
    try:
        request_id = int(request_id_str)
    except:
        await callback.answer(f"❌ Неверный ID: {request_id_str}", show_alert=True)
        return
    
    logging.info(f"🔍 ===== ОТКЛОНЕНИЕ ЗАЯВКИ #{request_id} =====")
    ...
    try:
        ...
        if response.data:
            ...
            tariff_name = TARIFFS.get(data['tariff_key'], {}).get('name_ru', data['tariff_key'])
            ...
            for admin_id in ADMIN_IDS:
                try:
                    admin_text = f"""❌ <b>ЗАЯВКА #{request_id} ОТКЛОНЕНА!</b>
...
async def process_key_activation(message: Message, key_param: str, state: FSMContext):
    ...
    # === ПРОВЕРКА: КЛЮЧ НА СПИСАНИЕ ===
    if key_data.get('tariff_key') == 'withdraw':
        await process_withdraw_key_activation(message, key_data, state)
        return
    # ================================
    ...
    if user_response.data and user_response.data[0].get('ref_by'):
        ...
        if ref_amount > 0 and tariff_key != "test677":
            ...
            logging.info(f"💰 Начислено {ref_amount} ₽ рефереру {referrer_id} за активацию ключа у {user_id}")
    # ================================================================
...
# ==================================================
# АДМИН: СПИСАНИЕ БАЛАНСА
# ==================================================

@dp.callback_query(F.data == "admin_withdraw_balance")
async def admin_withdraw_balance(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("❌ Только для админов!", show_alert=True)
        return
    ...
    await state.set_state(AdminStates.waiting_for_withdraw_user)


@dp.message(AdminStates.waiting_for_withdraw_user)
async def process_withdraw_user(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        await message.answer("❌ Только для админов!")
        return
    ...
    try:
        if user_input.startswith("@"):
            entity = await bot.get_chat(user_input)
            user_id = entity.id
        else:
    ...
    # Проверяем в базе
    user_response = supabase.table('users')\
        .select('user_id, ref_balance, first_name, username')\
        .eq('user_id', user_id)\
        .execute()
    ...
    balance = float(user_data.get('ref_balance', 0))
    name = user_data.get('first_name', 'Неизвестно')
    username = user_data.get('username', '')
    
    await state.update_data(withdraw_user_id=user_id)
    
    await message.answer(
        f"👤 Пользователь: {name} (@{username})\n"
        f"🆔 ID: {user_id}\n"
        f"💰 Баланс: {balance:.2f} ₽\n\n"
        f"✍️ Введите сумму для списания:"
    )
    await state.set_state(AdminStates.waiting_for_withdraw_amount)


@dp.message(AdminStates.waiting_for_withdraw_amount)
async def process_withdraw_amount(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    user_id = data.get('withdraw_user_id')
    ...
    await message.answer(
        f"✅ <b>Ключ для списания создан!</b>\n\n"
        f"👤 Пользователь: {user_id}\n"
        f"💰 Сумма списания: {amount:.2f} ₽\n"
        f"🔑 Ключ: <code>{key}</code>\n"
        f"🔗 Ссылка: {link}\n\n"
        f"📌 Отправьте ссылку пользователю.\n"
        f"После активации ключа баланс спишется автоматически."
    )
    await state.clear()
...
async def process_withdraw_amount(message: Message, state: FSMContext):
    ...
    # Сохраняем ключ с типом "withdraw" и суммой
    supabase.table('subscription_keys').insert({
        'key': key,
        'tariff_key': 'withdraw',
        'duration_days': None,
        'created_by': message.from_user.id,
        'withdraw_amount': amount,  # нужно добавить колонку в таблицу!
        'target_user_id': user_id
    }).execute()
...
async def process_withdraw_key_activation(message: Message, key_data: dict, state: FSMContext):
    """Обрабатывает активацию ключа для списания баланса"""
    user_id = message.from_user.id
    username = message.from_user.username or "без username"
    
    # Получаем данные
    target_user_id = key_data.get('target_user_id')
    amount = key_data.get('withdraw_amount', 0)
    
    if target_user_id != user_id:
        await message.answer("❌ Этот ключ предназначен для другого пользователя!")
        return
    ...
    supabase.table('users')\
        .update({'ref_balance': new_balance})\
        .eq('user_id', user_id)\
        .execute()
    
    delete_subscription_key(key_data['key'])
    
    await message.answer(
        f"✅ <b>С баланса списано {amount:.2f} ₽</b>\n\n"
        f"💰 Новый баланс: {new_balance:.2f} ₽\n\n"
        f"Если деньги вам не выдали, напишите админу @kasgd"
    )
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                admin_id,
                f"💸 У @{username} (ID: {user_id}) списано {amount:.2f} ₽ с баланса\n"
...
@dp.callback_query(F.data.startswith("admin_withdraw_user_"))
async def admin_withdraw_from_request(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("❌ Только для админов!", show_alert=True)
        return
    ...
    if balance < 300:
        await callback.message.answer(
            ...
            f"💰 Баланс пользователя: {balance:.2f} ₽\n"
            f"📌 Минимальная сумма: 300 ₽"
        )
        return
    
    # Генерируем ключ списания
    key = ''.join(random.choices(string.ascii_lowercase + string.digits, k=32))
    ...
    try:
        await bot.send_message(
            ...
            f"💰 Сумма: {balance:.2f} ₽\n"
            f"🔗 Перейдите по ссылке для списания средств:\n{link}\n\n"
            f"После активации средства будут переведены администратором."
        )
    except Exception as e:
...
async def notify_admins(text: str, reply_markup: InlineKeyboardMarkup = None):
    """Безопасная отправка уведомлений админам"""
    for admin_id in ADMIN_IDS:
        try:
            ...
            try:
                ...
                if not chat:
                    logging.warning(f"⚠️ Чат с админом {admin_id} не найден!")
                    continue
            except Exception as e:
                logging.warning(f"⚠️ Не могу получить чат админа {admin_id}: {e}")
                continue
...
async def withdraw_request(callback: CallbackQuery, state: FSMContext):
    ...
    if balance < 300:
        await callback.answer(
            f"❌ Минимальная сумма для вывода: 300 ₽\n"
            f"💰 Ваш баланс: {balance:.2f} ₽\n\n"
    ...
    admin_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💸 Создать ключ списания", callback_data=f"admin_withdraw_user_{user_id}_{request_id}")]
    ])
    
    admin_text = f"""
🆕 <b>НОВАЯ ЗАЯВКА НА ВЫВОД!</b>

👤 Пользователь: <a href='tg://user?id={user_id}'>{first_name}</a>
🆔 ID: <code>{user_id}</code>
👥 Username: @{username}
💰 Баланс пользователя: {balance:.2f} ₽
📌 Сумма вывода: {balance:.2f} ₽
🆔 Заявка: #{request_id}
...
@dp.message(Command("debug_requests"))
async def debug_requests(message: Message):
    """Показывает все заявки для отладки"""
    if message.from_user.id not in ADMIN_IDS:
        await message.answer("❌ Только для админов!")
        return
    
    text = "🔍 <b>ОТЛАДКА ЗАЯВОК</b>\n\n"
    
    # 1. Проверяем таблицу в Supabase
    try:
        response = supabase.table('payment_requests')\
            .select('*')\
            .order('id', desc=True)\
        ...
        text += "📊 <b>Supabase (последние 10):</b>\n"
        if response.data:
            for req in response.data:
                text += f"  #{req['id']} | {req['username']} | {req['status']} | {req['created_at'][:16]}\n"
        else:
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute('SELECT id, user_id, username, status, created_at FROM payment_requests ORDER BY id DESC LIMIT 10')
        results = cursor.fetchall()
        conn.close()
        
        text += "💾 <b>SQLite (последние 10):</b>\n"
        if results:
            for req in results:
                text += f"  #{req[0]} | {req[2]} | {req[3]} | {req[4][:16]}\n"
        else:
            text += "  ❌ Пусто\n"
    except Exception as e:
        text += f"  ❌ Ошибка: {e}\n"
...
@dp.message(Command("find_request"))
async def find_request(message: Message):
    ...
    try:
        ...
        await message.answer("❌ Использование: /find_request <id>")
        return
    
    text = f"🔍 <b>ПОИСК ЗАЯВКИ #{request_id}</b>\n\n"
    ...
    try:
        ...
        if response.data:
            ...
            text += f"  🏷️ tariff_key: {data['tariff_key']}\n"
            text += f"  💰 amount: {data['amount']}\n"
            text += f"  📊 status: {data['status']}\n"
            text += f"  📅 created_at: {data['created_at']}\n"
        else:
            text += "  ❌ НЕ НАЙДЕНА\n"
    except Exception as e:
        text += f"  ❌ Ошибка: {e}\n"
    ...
    try:
        ...
        if result:
            text += f"  ✅ НАЙДЕНА!\n"
            text += f"  🆔 ID: {result[0]}\n"
            text += f"  👤 user_id: {result[1]}\n"
            text += f"  📋 username: {result[2]}\n"
            text += f"  📊 status: {result[8]}\n"
        else:
            text += "  ❌ НЕ НАЙДЕНА\n"
    except Exception as e:
        text += f"  ❌ Ошибка: {e}\n"
...
Напишите админу @kasgd для получения реквизитов
...
async def admin_payment_requests(callback: CallbackQuery):
    ...
    for req in requests:
        ...
        text += f"{status_emoji} #{req['id']} | Пользователь: {req['username']} | {req['amount']} RUB | {req['created_at'][:16]}\n"
...
📎 Медиа: {req[6] or "Нет"}
📅 Создана: {req[9][:19] if req[9] else "Неизвестно"}
📊 Статус: {req[8]}
"""
...
async def confirm_payment(callback: CallbackQuery):
    ...
    # Извлекаем ID заявки
    request_id_str = callback.data.replace("confirm_payment_", "")
    try:
        request_id = int(request_id_str)
    except:
        await callback.answer(f"❌ Неверный ID: {request_id_str}", show_alert=True)
    ...
    logging.info(f"🔍 ===== ПОДТВЕРЖДЕНИЕ ЗАЯВКИ #{request_id} =====")
    ...
    try:
        ...
        if response.data:
            ...
            # ===== 3. ОТПРАВЛЯЕМ УВЕДОМЛЕНИЕ ПОЛЬЗОВАТЕЛЮ (БЕЗ КЛЮЧА) =====
            text = f"""✅ <b>Оплата подтверждена!</b>

📋 Тариф: {tariff_name}
📅 Действует до: {expires_text}

❕Ваш тариф уже появился в разделе <b>«Мои подписки»</b>
